...
- Chrome DevTools MCP documentation with setup guide and troubleshooting

### Changed

- Remove `.claude/` and `CLAUDE.md` from git tracking (now in .gitignore)

## 0.4.1 - 2025-12-11

### Changed

- Simplified skill names by removing `skill-` prefix (e.g., `browser-use` instead of `skill-browser-use`)

## 0.4.0 - 2025-12-11

### Added

- `product-aio` skill for product management, feature planning, and documentation
...
- Business process documentation for account creation (Cloudflare email routing)

## 0.3.6 - 2025-12-09

### Changed

- `/commit-code` command now removes `## [Unreleased]` section after release
...
- Install location selection popup (global/local) when pressing Enter in Skills view
- Quick install shortcuts (`g` for global, `l` for local) in Skills view
- Delete skill action (`d` key) in Skills view
